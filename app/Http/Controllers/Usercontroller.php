...
class Usercontroller extends Controller
{
    ...
    public function getChitiet(Request $req)
    {
        ...
        if (Gallery::where('gallery_product_id', $sanpham->id)->first()) {

            $gallery_asc = Gallery::where('gallery_product_id', $sanpham->id)->orderBy('gallery_id', 'asc')->first();
            $gallery = Gallery::where('gallery_product_id', $sanpham->id)->where('gallery_id', '!=', $gallery_asc->gallery_id)->get();
        } else {
            $gallery_asc = '';
            $gallery = '';
        }
        ...
        $rating = round($rating);
        return view('FrontEnd.DetailProduct', compact('sanpham', 'tuongtu', 'new_product', 'sanpham_khuyenmai', 'meta_desc', 'image_og', 'rating', 'sanpham_id', 'gallery', 'gallery_asc'));
    }
